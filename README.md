...
3. (Optional) Tune performance with these environment variables:
   - `LLAMA_CPP_THREADS` — number of CPU threads to use. Set to your physical core count for best performance; `0` (default) = auto-detect via `os.cpu_count()` (fallback: 4).
...
4. Launch with env vars set (`MCFG_LLM` can be left empty when `LOCAL_GGUF_MODEL` is set — HF downloads are skipped entirely):

   **Linux/macOS:**
   ```bash
   LOCAL_GGUF_MODEL=/path/to/your_model.q6_k.gguf \
   LLAMA_CPP_THREADS=8 \
