...
3. (Optional) Tune performance with these environment variables:
   - `LLAMA_CPP_THREADS` — number of CPU threads to use. Set to your physical core count for best performance; `0` (default) = auto-detect (`os.cpu_count()`), falling back to `4` if detection fails.
...
4. Launch (HF downloads will be skipped entirely; `MCFG_LLM` may be left empty or unset):
   ```
   # Windows
   ...
   # Linux/macOS
   LOCAL_GGUF_MODEL=/path/to/your_model.q6_k.gguf MCFG_LLM= python enhanced_rag_system.py
   ```
